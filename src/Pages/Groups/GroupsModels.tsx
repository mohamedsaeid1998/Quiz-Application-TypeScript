...
import {
  ...
  DeleteModel,
  EditModel,
} from "@/Components/Shared/Models/Models";
  ...
  return (
    <>
      <EditModel title="Update Group" {...{ isOpenEditModel, closeModalEdit }}>
        ...
        {!loadingData && (
          <form onSubmit={handleSubmitEdit(handleEditGroup)}>
            <GroupInput
              className="mt-5"
              {...editRegister("name", FieldValidation)}
              label="Group Name"
            />
            ...
            >
              ...
              <select
                id={"select"}
                {...editRegister("students", FieldValidation)}
                multiple
                className="px-2 rounded-r-md outline-none flex-1 border-none  bg-transparent py-1.5 pl-1 text-black placeholder:text-gray-400  sm:text-sm sm:leading-6"
              >
