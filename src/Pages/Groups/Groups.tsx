...
import {
  ...
  useGroupsListQuery,
} from "@/Redux/Services/Groups/GroupsSlice";
import {
  useAllStudentsQuery,
  useAllStudentsWithoutGroupsQuery,
} from "@/Redux/Services/Students/StudentsSlice";
...
const Groups = () => {
  ...
  const { data: studentsData, refetch: studentsRefetch } =
    useAllStudentsWithoutGroupsQuery(0);
  //* ***************Create New Group ***************
  const [isOpen, toggleOpen] = useToggle();
  const openModal = () => toggleOpen();
  const closeModal = () => {
    toggleOpen();
  };
  ...
  const [isOpenDeleteModel, toggleOpenDeleteModel] = useToggle();
  const openModalDelete = (_id: string) => {
    toggleOpenDeleteModel();
    setDeleteItem(_id);
  };
  ...
  const [isOpenEditModel, toggleOpenEditModel] = useToggle();

  const closeModalEdit = () => {
    toggleOpenEditModel();
    setEditItem("");
  };
  const [loadingData, toggleLoadingData] = useToggle();
  ...
  useEffect(() => {
    ...
    setValue(
      "students",
      groupData?.students?.map(({ _id }: any) => {
        return _id;
      })
    );
  }, [groupData?.name]);
  ...
  return (
    <>
      <AddGroupModal
        {...{ isOpen, closeModal, studentsData, studentsRefetch }}
      />
      <DeleteGroupModal
        {...{
          closeModalDelete,
