...
export const DetailsInput = forwardRef(({ className = "", label, content, icon, ...rest }: IProps, ref: Ref<HTMLParagraphElement>) => {
  ...
  return <>
    <div className={`${appliedClass} flex items-center border-2 rounded-lg `}>
      <label className='bg-secondColor p-2 font-semibold  flex justify-center min-w-20'>
        {label}
      </label>
      <p ref={ref} title={content} className=" pl-3 text-black flex flex-1 py-1.5 ms-1 truncate  " {...rest} >
